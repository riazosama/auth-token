...
class AuthController implements IControllerBase {
  ...
  async login(req: Request | any, res: Response) {
    ...
    if (userAccount) {
      payload = { email: userAccount.email };
  ...
  async getUserProfile(req: Request, res: Response) {
    ...
    try {
      ...
      const userAccount = accounts.find(account => account.email === decoded.email);
      if (userAccount) {
  ...
  async logout(req: Request, res: Response) {
    ...
    if (userAccount) {
      await authToken.removeTokenForDevice(userAccount.id, req.headers['user-agent']);
